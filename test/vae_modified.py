...
from src.read_datasets import read_datasets2, read_datasets
#normal_data=read_datasets2('../data/data3.pickle')
normal_data=read_datasets('../data/DB_005V0_l100.pickle')
#normal_data=read_datasets('../data/bignormal1001cpu.pickle','../data/labelsbignormal1001cpu.pickle')
#anomalous_data=read_datasets('../data/biganomalous1001cpu.pickle','../data/labelsbiganomalous1001cpu.pickle')
n_samples = normal_data.train.num_examples
print('n_samples:',n_samples)
n_samples_test = normal_data.test.num_examples
...
class VariationalAutoencoder(object):
    ...
    def _create_loss_optimizer(self):
        ...
        reconstr_loss = -tf.reduce_sum(self.x * tf.log(1e-5 + self.x_reconstr_mean)
...
def train(network_architecture, sess, learning_rate=0.001,
    ...
    vae = VariationalAutoencoder(network_architecture, sess,
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            batch_xs, _ = normal_data.train.next_batch(batch_size)
...
with tf.Session() as sess:
    ...
    batch_xs, _ = normal_data.test.batch_test(normal_data.test.patterns[0])
    batch_size = len(batch_xs)
    vae = train(network_architecture, sess, training_epochs=100, batch_size=batch_size)
    ...
    # In[7]:
    umbral_normal = -np.inf
    umbral_fail = np.inf
    for pattern in normal_data.test.patterns:
        batch_xs, _ = normal_data.test.batch_test(pattern)
        metric = vae.test_vae(batch_xs)
        print(pattern,':',metric)
        if pattern[3] == 1:
            if metric > umbral_normal:
                umbral_normal = metric
        else:
            if metric < umbral_fail:
                umbral_fail = metric

    print('distance:', umbral_fail-umbral_normal)